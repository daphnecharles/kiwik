...
const makeMerkleTreeProof = (leaves, index) => {
    ...
    const tree = new merkletreejs_1.MerkleTree(bufLeaves, (el) => {
        return core_utils_1.fromHexString(ethers_1.ethers.utils.keccak256(el));
    });
    const proof = tree.getProof(bufLeaves[index], index).map((element) => {
        return core_utils_1.toHexString(element.data);
    });
    return proof;
};
...
const makeStateTrieProof = async (provider, blockNumber, address, slot) => {
    const proof = await provider.send('eth_getProof', [
        address,
        [slot],
        core_utils_1.toRpcHexString(blockNumber),
    ]);
    return {
        accountProof: core_utils_1.toHexString(rlp.encode(proof.accountProof)),
        storageProof: core_utils_1.toHexString(rlp.encode(proof.storageProof[0].proof)),
    };
};
